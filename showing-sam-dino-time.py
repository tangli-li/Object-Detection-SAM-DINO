...
from small_model import minimodel
...
def get_args_parser():
    ...
    return parser
class FrameQue:
    def __init__(self,post_frame=3):
        self.hs=[]
        self.reference=[]
    ...
    def clear(self):
        self.hs=[]
        self.reference=[]
    def append(self,hs0,reference0):
        if len(self.hs)>=self.post_frame:
            self.hs.pop(0)
        self.hs.append(hs0)
        if len(self.reference)>=self.post_frame:
            self.reference.pop(0)
        self.reference.append(reference0)
    def show(self,hs0,reference0):#hs0,reference0为当前帧
        hs_out=self.hs.copy()
        reference_out=self.reference.copy()
        while len(hs_out)<self.post_frame+1 and len(reference_out)<self.post_frame+1:
            hs_out.append(hs0)
            reference_out.append(reference0)
        return hs_out,reference_out
...
def main(args,compare=1):
    ...
    pretrain_mini_model = "/media/mldadmin/home/s125mdg35_05/DINO/DINO/output_mini_702/temporal_module.pth"
    ...
    model_post = minimodel(dec_num=model_DINO.transformer.num_decoder_layers, post_frames=3,
    ...
    # ------------------------------------打印出网络的参数大小
    n_parameters = sum(p.numel() for p in model_DINO.parameters() if p.requires_grad)
    print("DINO parameters:", n_parameters)

    m_parameters = sum(p.numel() for p in model_post.parameters() if p.requires_grad)
    print("temporal module parameters:", m_parameters)
    ...
    for i in range(17800,18000):
        ...
        if compare!=1:
            ...
            with torch.no_grad():  # 禁用梯度计算以节省内存
                hs0, reference0 = model_DINO(image_tensor, targets=None, return_ref=True)
                hs_input,reference_input = Que.show(hs0, reference0)
                inference_result = model_post(reference_input,hs_input)
                Que.append(hs0,reference0)
